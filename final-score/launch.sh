...
API_PORT="${FINAL_SCORE_API_PORT:-8290}"
API_URL="http://127.0.0.1:${API_PORT}/healthz"
RUN_DIR="/tmp/final-score"
API_PID_FILE="${RUN_DIR}/final-score-api.pid"
API_LOG_FILE="${RUN_DIR}/final-score-api.log"
...
start_api() {
  stop_pidfile_process "${API_PID_FILE}" "final-score api"
  stop_port_listener "${API_PORT}"
  ensure_runtime

  nohup env \
    PYTHONPATH="${PROJECT_DIR}/src" \
    FINAL_SCORE_API_PORT="${API_PORT}" \
    "${VENV_PY}" -m uvicorn final_score.api:app --host 0.0.0.0 --port "${API_PORT}" >"${API_LOG_FILE}" 2>&1 &

  local api_pid=$!
  echo "${api_pid}" > "${API_PID_FILE}"
  echo "Starting final-score api on port ${API_PORT} (${api_pid})"
  wait_http_ready "${API_URL}" 90 "${api_pid}"
  echo "Final Score API log: ${API_LOG_FILE}"
}
...
show_status() {
  if [[ -f "${API_PID_FILE}" ]]; then
    ...
    if [[ -n "${pid}" ]] && kill -0 "${pid}" >/dev/null 2>&1; then
      echo "Final Score API: running (pid=${pid}, port=${API_PORT})"
      return 0
    fi
  fi

  if ss -ltn "sport = :${API_PORT}" 2>/dev/null | grep -q ":${API_PORT}"; then
    echo "Final Score API: listening on port ${API_PORT}"
  else
    echo "Final Score API: not running"
  fi
}
...
case "${ACTION}" in
  ...
  logs)
    if [[ -f "${API_LOG_FILE}" ]]; then
      ...
      echo "No final-score API log file yet: ${API_LOG_FILE}"
    fi
    ;;
  *)
